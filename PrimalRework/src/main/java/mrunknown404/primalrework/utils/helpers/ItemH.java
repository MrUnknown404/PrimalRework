...
public class ItemH {
	private static final Cache<ItemStack, List<ITextComponent>> TOOLTIP_CACHE = Cache.create((k0, k1) -> k0 == null || k1 == null ? false : k0.equals(k1, false));
			...
			if (stage.hasAccessToCurrentStage()) {
				...
				if (item instanceof SIFood && PRConfig.CLIENT.tooltips_showFoodNutrients.get()) {
					...
					if (item instanceof SIDamageable && config.tooltips_showDurability.get()) {
						...
						list.add(WordH.string((di.toolMat.durability - stack.getDamageValue()) + "/" + di.toolMat.durability + " ")
								.append(WordH.translate("tooltips.stat.durability")).withStyle(TextFormatting.GRAY));
					}
