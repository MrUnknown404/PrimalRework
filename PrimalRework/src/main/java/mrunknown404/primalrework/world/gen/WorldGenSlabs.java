...
public class WorldGenSlabs extends WorldGenerator {
	...
	public boolean generate(World w, Random r, BlockPos pos) {
		...
		if ((b == Blocks.GRASS || b == Blocks.SNOW_LAYER || b == ModBlocks.MUSHROOM_GRASS) && w.getBlockState(pos.up()).getMaterial().isReplaceable()) {
			BlockPos toCheck = b == Blocks.GRASS || b == ModBlocks.MUSHROOM_GRASS ? pos.up() : pos;
			int amount = 0;
			...
			if (amount >= 2) {
				if (b == Blocks.GRASS || b == ModBlocks.MUSHROOM_GRASS) {
					w.setBlockState(pos.up(), (b == Blocks.GRASS ? ModBlocks.GRASS_SLAB : ModBlocks.MUSHROOM_GRASS_SLAB).getStateFromMeta(0), 2);
