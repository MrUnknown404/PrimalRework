...
public class BlockClayFurnace extends BlockStagedDirectional implements ITileEntityProvider {
	...
	public BlockClayFurnace() {
		super("clay_furnace", Material.ROCK, SoundType.STONE, BlockRenderLayer.CUTOUT_MIPPED, 2f, 4f, bb, bb, EnumStage.stage3,
	...
	public void randomDisplayTick(IBlockState state, World w, BlockPos pos, Random r) {
		...
		if (te instanceof TileEntityClayFurnace) {
			if (((TileEntityClayFurnace) te).isBurning()) {
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess w, BlockPos pos) {
		return getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(IS_BURNING, ((TileEntityClayFurnace) w.getTileEntity(pos)).isBurning());
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (!worldIn.isRemote) {
			playerIn.openGui(Main.main, GuiHandler.GuiID.CLAY_FURNACE.toID(), worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
	...
	public void breakBlock(World world, BlockPos pos, IBlockState state) {
		...
		if (te instanceof TileEntityClayFurnace) {
			InventoryHelper.dropInventoryItems(world, pos, (TileEntityClayFurnace) te);
