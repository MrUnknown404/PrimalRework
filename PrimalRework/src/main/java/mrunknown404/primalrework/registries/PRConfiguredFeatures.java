...
public class PRConfiguredFeatures {
	...
	private static final BlockState S_SHORT_GRASS = PRBlocks.SHORT_GRASS.get().defaultBlockState();
	private static final BlockState S_MEDIUM_GRASS = PRBlocks.MEDIUM_GRASS.get().defaultBlockState();
	...
	private static final BlockClusterFeatureConfig C_SHORT_GRASS = new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(S_SHORT_GRASS), SimpleBlockPlacer.INSTANCE).tries(32).build();
	private static final BlockClusterFeatureConfig C_MEDIUM_GRASS = new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(S_MEDIUM_GRASS), SimpleBlockPlacer.INSTANCE).tries(32).build();
	private static final BlockClusterFeatureConfig C_TALL_GRASS = new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(S_TALL_GRASS), SimpleBlockPlacer.INSTANCE).tries(32).build();
	
	public static final ConfiguredFeature<?, ?> OAK_COMMON = register("oak_common",PRFeatures.TREE.get().configured(C_OAK).decorated(Placements.HEIGHTMAP_SQUARE).decorated(Placement.COUNT_EXTRA.configured(new AtSurfaceWithExtraConfig(8, 0.2f, 1))));
	public static final ConfiguredFeature<?, ?> BIG_OAK_RARE = register("big_oak_rare", PRFeatures.TREE.get().configured(C_BIG_OAK).decorated(Placements.HEIGHTMAP_SQUARE).decorated(Placement.COUNT_EXTRA.configured(new AtSurfaceWithExtraConfig(2, 0.2f, 1))));
	public static final ConfiguredFeature<?, ?> BIRCH_RARE = register("birch_rare", PRFeatures.TREE.get().configured(C_BIRCH).decorated(Placements.HEIGHTMAP_SQUARE).decorated(Placement.COUNT_EXTRA.configured(new AtSurfaceWithExtraConfig(2, 0.2f, 1))));
	public static final ConfiguredFeature<?, ?> SUNKEN_OAK_COMMON = register("sunken_oak_common", PRFeatures.TREE.get().configured(C_SUNKEN_OAK).decorated(Placements.HEIGHTMAP_SQUARE).decorated(Placement.COUNT_EXTRA.configured(new AtSurfaceWithExtraConfig(8, 0.2f, 1))));
	public static final ConfiguredFeature<?, ?> SUNKEN_BIG_OAK_RARE = register("sunken_big_oak_rare", PRFeatures.TREE.get().configured(C_SUNKEN_BIG_OAK).decorated(Placements.HEIGHTMAP_SQUARE).decorated(Placement.COUNT_EXTRA.configured(new AtSurfaceWithExtraConfig(2, 0.2f, 1))));
	public static final ConfiguredFeature<?, ?> SUNKEN_BIRCH_RARE = register("sunken_birch_rare", PRFeatures.TREE.get().configured(C_SUNKEN_BIRCH).decorated(Placements.HEIGHTMAP_SQUARE).decorated(Placement.COUNT_EXTRA.configured(new AtSurfaceWithExtraConfig(2, 0.2f, 1))));
	public static final ConfiguredFeature<?, ?> GROUND_SLABS = register("ground_slabs", PRFeatures.GROUND_SLABS.get().configured(new NoFeatureConfig()).decorated(Placements.HEIGHTMAP_WORLD_SURFACE));
	public static final ConfiguredFeature<?, ?> GROUND_ITEMS = register("ground_items", PRFeatures.GROUND_ITEMS.get().configured(new NoFeatureConfig()).decorated(Placements.HEIGHTMAP_SQUARE).decorated(Placement.COUNT_EXTRA.configured(new AtSurfaceWithExtraConfig(1, 0.5f, 2))));
